...
static inline void LogAdapterMonitors(IDXGIAdapter1 *adapter, int adapter_index)
{
	UINT i;
	ComPtr<IDXGIOutput> output;
	...
	for (i = 0; adapter->EnumOutputs(i, &output) == S_OK; ++i) {
		...
		monitor.friendly_name = friendly_name;
		monitor.left = rect.left;
		monitor.top = rect.top;
		monitor.cx = rect.right - rect.left;
		monitor.cy = rect.bottom - rect.top;
		monitor.dpi = dpiX / 96.0f;
		monitor.attachedToDesktop = !!desc.AttachedToDesktop;
		monitor.index = i;
		...
		graphics_information_obj.monitors.push_back(monitor);
	}
}
...
static inline void LogD3DAdapters()
{
	...
	for (i = 0; factory->EnumAdapters1(i, adapter.Assign()) == S_OK; ++i) {
		...
		if (auto hags_support = GetAdapterHagsStatus(&desc)) {
			...
			adpater.hags_enabled = hags_support->enabled;
			adpater.hags_enabled_by_default= hags_support->enabled_by_default;
		} else {
		...
		if (SUCCEEDED(hr)) {
			...
			adpater.driver = version_str;
				   
		} else {
			blog(LOG_INFO, "\t  Driver Version: Unknown (0x%X)",
			     (unsigned)hr);
			adpater.driver = "Unknown";
		}

		graphics_information_obj.adapters.push_back(adpater);

		LogAdapterMonitors(adapter, i/*adpater.index*/);
	}
}
...
{
	if (refresh) {
		blog(LOG_INFO, "refresh graphics inforamtion");
		LogD3DAdapters();
	}
	return &graphics_information_obj;
}
