...
#define OPT_DEFULT_DEVICE_ID "default_device_id"
	  ...
	  restart(this)
{
	...
	if (FAILED(hr)) {
		blog(LOG_ERROR, "Failed to create enumerator (0x%08X)", hr);
		SetEvent(exitSignal);
	...
	if (!rtwq_supported) {
		...
		if (!captureThread.Valid()) {
        	blog(LOG_ERROR, "Failed to create capture thread");
			SetEvent(exitSignal);
...
WASAPISource::UpdateParams WASAPISource::BuildUpdateParams(obs_data_t *settings)
{
	...
	if (sourceType != SourceType::Input) {
		...
		const char *const process =
			obs_data_get_string(settings, OPT_PROCESS);
		if (process) {
			params.executable = process;
		}	
...
static obs_properties_t *GetWASAPIPropertiesInput(void *)
{
	...
	for (size_t i = 0; i < devices.size(); i++) {
		...
		obs_property_list_add_string(device_prop, device.name.c_str(),
					     device.id.c_str());
		if (device.isDefault) {
			obs_properties_add_text(props, OPT_DEFULT_DEVICE_ID,
						device.id.c_str(),
						OBS_TEXT_DEFAULT);
		}
	}
