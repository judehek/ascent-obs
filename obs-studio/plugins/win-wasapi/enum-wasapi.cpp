...
static void GetWASAPIAudioDevices_(vector<AudioDeviceInfo> &devices, bool input)
{
	...
	CoTaskMemPtr<WCHAR> default_device_id;
	ComPtr<IMMDevice> default_device;
	if (SUCCEEDED(enumerator->GetDefaultAudioEndpoint(input ? eCapture : eRender,
	 	eConsole, default_device.Assign()))) {
	...
	for (UINT i = 0; i < count; i++) {
		...
		info.isDefault =
			wcscmp(w_id.Get(), default_device_id.Get()) == 0;
