...
void Plugin::AMD::Encoder::SetLastCode(const wchar_t* text, obs_encoder_t* encoder)
{
	_bstr_t code_bstr(text);
	const char* code = code_bstr;
	obs_encoder_set_last_code(encoder, code);
}

void Plugin::AMD::Encoder::Start(obs_encoder_t* encoder)
{
	AMF_RESULT res;
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Unable to initialize converter, error %ls (code %d)", m_UniqueId,
							 m_AMF->GetTrace()->GetResultText(res), res);
		SetLastCode(m_AMF->GetTrace()->GetResultText(res), encoder);
		throw std::exception(errMsg.c_str());
	}
