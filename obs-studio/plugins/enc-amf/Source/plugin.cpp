...
MODULE_EXPORT bool obs_module_load(void)
{
	try {
		...
		// Out-of-process AMF Test
		{
			...
			// prevent for loading hang on some machines
			std::thread reader_thread{[hRead] {
				char  buf[1024];
				DWORD bufread = 0;
				try {
			...
			if (WaitForSingleObject(hProcess, 5000) == WAIT_OBJECT_0){
			   ...
			   PLOG_ERROR("AMD TEST time out, terminate process.");
		           TerminateProcess(hProcess, returnCode);
			}

			TerminateThread(reader_thread.native_handle(), 0);
