...
RequestResult RequestHandler::SetVideoSettings(const Request &request)
{
	...
	bool changeFps1 = request.Contains("fps");
	if (changeFps1 && !(request.ValidateOptionalNumber("fps", statusCode, comment, 1))) {
		return RequestResult::Error(statusCode, comment);
	}

	config_t *config = owobs_controller_->Config();
	SetOptionalStringValue(config, request, "Video", "colorFormat", "ColorFormat");
	SetOptionalStringValue(config, request, "Video", "colorRange", "ColorRange");
	SetOptionalStringValue(config, request, "Video", "colorSpec", "ColorSpec");
	...
	} else if (changeFps1) { // by int
		config_set_uint(config, "Video", "FPSType", 0);
		config_set_uint(config, "Video", "FPSInt", request.RequestData["fps"]);
	}
	...
	if (changeBaseRes) {
		...
		config_set_uint(config, "Video", "OutputCX", request.RequestData["baseWidth"]);
		config_set_uint(config, "Video", "OutputCY", request.RequestData["baseHeight"]);
	}
	...
	if (request.Contains("sdrWhite")) {
		config_set_uint(config, "Video", "SdrWhiteLevel", request.RequestData["sdrWhite"]);
	}

	if (request.Contains("hdrPeak")) {
		config_set_uint(config, "Video", "HdrNominalPeakLevel", request.RequestData["hdrPeak"]);
	}


	auto res = owobs_controller_->ResetVideo();
	if (res != OBS_VIDEO_SUCCESS) {
		return RequestResult::Error(RequestStatus::GenericError, "reset setting error: " + std::to_string(res));
	}
