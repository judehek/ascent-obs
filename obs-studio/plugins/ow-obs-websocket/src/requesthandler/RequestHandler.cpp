...
const std::unordered_map<std::string, RequestMethodHandler> RequestHandler::_handlerMap{
	...
	// ow-obs
	{"QueryInformation", &RequestHandler::QueryInformation},
	{"SetAudioSettings", &RequestHandler::SetAudioSettings},
    {"SetAudioDevices", &RequestHandler::SetAudioDevices},
	{"AddMonitorSource", &RequestHandler::AddMonitorSource},
	{"AddSource", &RequestHandler::AddSource},
...
RequestHandler::RequestHandler(OWOBSControllerPtr owobs_controller, SessionPtr session) : 
	owobs_controller_(owobs_controller), _session(session)
{
}
