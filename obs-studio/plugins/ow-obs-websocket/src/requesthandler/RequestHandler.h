...
public:
	...
	RequestHandler(OWOBSControllerPtr owobs_controller, SessionPtr session);
...
private:
	...
	// ow-obs
	RequestResult QueryInformation(const Request &);
	RequestResult SetAudioSettings(const Request &);
	RequestResult SetAudioDevices(const Request &);
	RequestResult AddSource(const Request &);
	RequestResult AddMonitorSource(const Request &);
	...
	OWOBSControllerPtr owobs_controller_ = nullptr;
