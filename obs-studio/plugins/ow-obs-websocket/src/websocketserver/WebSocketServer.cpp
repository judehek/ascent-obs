...
WebSocketServer::WebSocketServer(OWOBSControllerPtr &obs_controller)
:obs_controller_(obs_controller) {
	;
...
bool WebSocketServer::Start()
{
	...
	std::string port = std::to_string(conf->ServerPort);
	_server.listen(local_host, port, errorCode);

	if (errorCode) {
		std::string errorCodeMessage = errorCode.message();
		blog(LOG_INFO, "[WebSocketServer::Start] Listen failed [port% s]: %s", 
			port, errorCodeMessage.c_str());
		return false;
	}
...
void WebSocketServer::onOpen(websocketpp::connection_hdl hdl)
{
	...
	session->SetConnectedAt(Utils::Time::CurrentSecsSinceEpoch());
	...
	helloMessageData["obsWebSocketVersion"] = OWOBS_WEBSOCKET_VERSION;
	helloMessageData["rpcVersion"] = OWOBS_WEBSOCKET_RPC_VERSION;
