...
void AddPropertyToJson(obs_property_t *property, json &outJson, json *defaults)
{
	...
	json proprty_info;
	proprty_info["description"] = desc;

	if (defaults && defaults->contains(name)) {
		proprty_info["default"] = (*defaults)[name];
	}
	...
	switch (type) {
		...
		case OBS_PROPERTY_EDITABLE_LIST: {
			...
			json values_desc = json::array();
			for (size_t idx = 0; idx < count; idx++) {
				json itemInfo;
				values_desc.push_back(obs_property_list_item_name(property, idx));
				if (format == OBS_COMBO_FORMAT_INT) {
					values.push_back(obs_property_list_item_int(property, idx));
				} else if (format == OBS_COMBO_FORMAT_FLOAT) {
					values.push_back(obs_property_list_item_float(property, idx));
				} else if (format == OBS_COMBO_FORMAT_STRING) {
					values.push_back(obs_property_list_item_string(property, idx));
				} else if (format == OBS_COMBO_FORMAT_BOOL) {
					values.push_back(obs_property_list_item_bool(property, idx));
				}
			}

			proprty_info["values"] = values;
			proprty_info["valuesDesc"] = values_desc;			
			break;
		}
	}

	outJson[name] = proprty_info;
}

json Utils::Json::ObsPropsToJson(obs_properties_t *properties, json *defaults)
{
	json j = json::object();
	if (!properties)
		return j;
	...
	while (property) {
		AddPropertyToJson(property, j, defaults);
