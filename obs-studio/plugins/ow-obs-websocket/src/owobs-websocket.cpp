...
#include "obs-controller/owobs-controller.hpp"
#include <util/command_line.hpp>

OBS_DECLARE_MODULE()
OBS_MODULE_USE_DEFAULT_LOCALE("owobs-websocket", "en-US")
OBS_MODULE_AUTHOR("ow-electron")

const char *obs_module_name(void)
{
	return "ow-obs-websocket";
}

const char *obs_module_description(void)
{
	return "ow-obs remote controller";
}
...
OWOBSControllerPtr obs_controller_;
...
bool obs_module_load(void)
{
	...
	blog(LOG_INFO, "[obs_module_load] you can haz websockets (Version: %s | RPC Version: %d)", OWOBS_WEBSOCKET_VERSION,
	     OWOBS_WEBSOCKET_RPC_VERSION);
	...
	obs_controller_ = OWOBSControllerPtr(new OWOBSController());

	// Initialize the event handler
	_eventHandler = EventHandlerPtr(new EventHandler());

	// Initialize the plugin/script API
	_webSocketApi = WebSocketApiPtr(new WebSocketApi(obs_controller_));
	_webSocketApi->SetEventCallback(WebSocketApiEventCallback);

	// Initialize the WebSocket server
	_webSocketServer = WebSocketServerPtr(new WebSocketServer(obs_controller_));
...
void obs_module_post_load(void)
{
	...
	if (!obs_controller_->Init()) {
		blog(LOG_ERROR, "[owobs websocket] fail to init obs controller");
		return;
	}

	auto commnad_line = CommandLine::ForCurrentProcess();
	if (commnad_line->HasSwitch(L"show-debug-window")) {
		obs_controller_->CreateDebugWindow();
	}

	blog(LOG_INFO, "[owobs websocket] WebSocket server is enabled, starting...");
