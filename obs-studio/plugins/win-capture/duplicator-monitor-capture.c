...
static BOOL CALLBACK enum_monitor_by_handle(HMONITOR handle, HDC hdc,
					    LPRECT rect, LPARAM param)
{
	UNUSED_PARAMETER(hdc);
	...
	if (GetMonitorInfoA(handle, (LPMONITORINFO)&mi)) {
		...
		if (EnumDisplayDevicesA(mi.szDevice, 0, &device,
					EDD_GET_DEVICE_INTERFACE_NAME)) {
			match = monitor->handle && monitor->handle == handle;
			if (match) {
				monitor->valid = true;
				monitor->rect = *rect;
				monitor->handle = handle;
				strcpy_s(monitor->device_id,
					 _countof(monitor->device_id),
					 mi.szDevice);
				GetMonitorName(handle, monitor->name,
					       _countof(monitor->name));
			}
...
/* ------------------------------------------------------------------------- */
static bool get_monitor_props_by_handle(int monitor_idx, HMONITOR* out_monitor_handle) {

  struct gs_monitor_info info;

  if (out_monitor_handle == NULL)
    return false;

  if (!gs_get_duplicator_monitor_info(monitor_idx, &info))
    return false;

  (*out_monitor_handle) = (HMONITOR)(uintptr_t)info.handle;
...
static struct duplicator_monitor_info find_monitor_by_handle(HMONITOR monitor_handle)
{
	struct duplicator_monitor_info monitor = {0};
	monitor.index = -1;
	monitor.handle = monitor_handle;
	EnumDisplayMonitors(NULL, NULL, &enum_monitor_by_handle, (LPARAM)&monitor);
	if (!monitor.valid) {
		monitor.index = -1;
		EnumDisplayMonitors(NULL, NULL, &enum_monitor_fallback,
				    (LPARAM)&monitor);
				   ...
				   obs_data_t *settings)
{
	pthread_mutex_lock(&capture->update_mutex);
      ...
      HMONITOR monitor_handle =
		(HMONITOR)obs_data_get_int(settings, "monitor_handle");

	struct duplicator_monitor_info monitor =
		find_monitor(obs_data_get_string(settings, "monitor_id"));

	if (monitor.handle == NULL && monitor_handle != NULL) {
		struct duplicator_monitor_info monitor1 =
			find_monitor_by_handle(monitor_handle);
		if (monitor1.valid) {
			monitor = monitor1;
			capture->handle = monitor_handle;
			info("found monitor id [%d] by handle [%d] (id: %s)",
			     monitor.index, monitor_handle, monitor.device_id);
		}           
	}
