...
BOOL CALLBACK enum_windows_callback1(HWND handle, LPARAM lParam)
{
	WINDOW_INFO *window_info = (WINDOW_INFO *)lParam;
	unsigned long process_id = 0;
	auto thread_id = GetWindowThreadProcessId(handle, &process_id);
	if (window_info->process_id == process_id) {
		if (window_info->main_window) {
			return TRUE;
		}
		window_info->window = handle;
		window_info->main_window = !!is_main_window(handle);
...
DWORD find_process_thread_id(DWORD process_id) {
	WINDOW_INFO window_info = {0};
	window_info.process_id = process_id;
	EnumWindows(enum_windows_callback1, (LPARAM)(&window_info));
	return window_info.thread_id;
}
...
static int inject_helper(wchar_t *argv[], const wchar_t *dll, int argc)
{
	DWORD id;
	...
	if (id == 0) {
		if (use_safe_inject && argc > 4) {
			DWORD process_id = wcstol(argv[4], NULL, 10);
			id = find_process_thread_id(process_id);
		}
		if (id == 0)
...
int main(void)
{
	...
	//MessageBoxA(0, "Inject helper", "Inject helper", 0);
	...
	if (argv) {
		if (argc >= 4) {
			if (GetModuleFileNameW(NULL, dll_path, MAX_PATH))
				ret = inject_helper(argv, argv[1], argc);
		}
