...
struct window_capture {
	...
	bool direct_Window_handle; /* direct game window handle for capture */
...
static void update_settings(struct window_capture *wc, obs_data_t *s)
{
   ...
   if (window && strcmp(window,"") != 0) {
	ms_build_window_strings(window, &wc->class, &wc->title,
				&wc->executable);

	wc->method = choose_method(method, wgc_supported, wc->class);
   } else {
		wc->window = (HWND)obs_data_get_int(s, "window_handle");
		if (wc->window) {
			wc->direct_Window_handle = true;
			wc->method = METHOD_WGC;
			blog(LOG_INFO, "window-capture using handle: %d",wc->window); 
		} else {
			blog(LOG_ERROR,"Can't get window handle for window capture");
		}	
	}
...
{
	// don't resest window handle in case of direct_Window_handle
	if (!wc->direct_Window_handle) {
		wc->window = NULL;
	}
