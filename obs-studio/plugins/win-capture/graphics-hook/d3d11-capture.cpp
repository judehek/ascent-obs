...
static void d3d11_init(IDXGISwapChain *swap)
{
	...
	if (global_hook_info->force_shmem) {
		...
		if (!success && (GetTickCount() - shtex_first_attempt) > SWITCH_TO_SHM_CAPTURE_FALLBACK_TIMEUOT) {
		    hlog_hr("d3d11_init: failed to init shtex, switch to shmem", hr);
...
bool capture_d3d11(void *swap_ptr, void *backbuffer_ptr) {
  if (!swap_ptr || !backbuffer_ptr)
    return false;

  if (!was_init) {
    on_demand_init_internal();
    return false;
  }

  d3d11_capture(swap_ptr, backbuffer_ptr);
  return true;
}
