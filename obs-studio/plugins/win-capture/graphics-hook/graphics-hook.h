...
typedef void(__cdecl *log_message_callback)(const char *message, int severity);
#ifdef _MSC_VER
...
enum CAPTURE_RESULT {
  CAPTURE_OK = 0,
  CAPTURE_FAIL,
  CAPUTER_SINGAL_INIT_ERROR,
  CAPUTER_SYSTEM_PATH_INIT_ERROR,
  CAPUTER_HOOK_INFO_INIT_ERROR,
  CAPUTER_MUTEX_INIT_ERROR,
  CAPUTER_HOOK_INIT_ERROR
};

extern bool was_init;
extern bool on_demand_init_reties ;
extern bool on_demand_init_reties;

EXPORT int on_demand_init(log_message_callback* callback);
int on_demand_init_internal();

void set_external_log_callback(log_message_callback  callback);

// d3d9
bool capture_d3d9(void* device, void* surface);
bool reset_d3d9();
// dx11
bool capture_d3d11(void *swap_ptr, void *backbuffer_ptr);
bool reset_d3d11();
// OGL
bool capture_ogl(HDC hdc);
bool reset_ogl(HGLRC hrc);
// Dx12
bool capture_d3d12(void *swap_ptr, void *queue_ptr);
bool reset_d3d12();


//
//VkQueue queue, const VkPresentInfoKHR *info vkimage
bool capture_vulkan(void *queue, const void *info, void *vk_image);
bool reset_vulkan(void* device);
...
extern void log_current_process(void);

static inline bool capture_should_init(void)
{
  if (!global_hook_info->ready_to_capture) {
    //hlog("not ready_to_capture");
    return false;
  }
