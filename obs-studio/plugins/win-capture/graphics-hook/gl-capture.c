...
#define DUMMY_WINDOW_CLASS_NAME L"ow_graphics_hook_gl_dummy_window"
...
bool capture_ogl(HDC hdc) {
  static bool gl_register_window_ = false;
  static bool is_winow_register = false;

  if (!gl_register_window_) {
    gl_register_window_ = true;
    gl = get_system_module("opengl32.dll");
    if (!gl) {
      return false;
    }
    is_winow_register = gl_register_window();
    
  }

  if (!is_winow_register)
    return false;
