...
struct vk_queue_data {
	...
	bool report_unsupports_transfer;
};
					    ...
					    struct vk_swap_data *swap)
{
	...
	if (use_gimr2 && funcs->GetImageMemoryRequirements2) {
	...
	if (use_bi2 && funcs->BindImageMemory2) {
			     ...
			     VkImage vk_image)
{
	...
	VkImage cur_backbuffer = vk_image ? vk_image
					  : swap->swap_images[image_index];
...
static void vk_capture(struct vk_data *data, VkQueue queue,
		       const VkPresentInfoKHR *info, VkImage vk_image)
{
					       ...
					       const VkPresentInfoKHR *info)
{
	...
	// Capture via OWCLIENT hook
	//if (data->valid && queue_data->supports_transfer) {
	//	vk_capture(data, queue, info, NULL);
	//}
					   ...
					   const VkAllocationCallbacks *ac)
{
	...
	PFN_vkDestroyInstance destroy_instance = ifuncs ? ifuncs->DestroyInstance : NULL;
...
{
	VkQueue queue = p_queue;
	const VkPresentInfoKHR *info = p_info;
	struct vk_data *const data = get_device_data_by_queue(queue);

	if (!data) {
		static bool log_missing_queue = true;
		if (log_missing_queue) {
			hlog("vulkan capture can't find queue 0x%x (device created?)", queue);
			log_missing_queue = false;
		}
	...
	if (data->valid && queue_data->supports_transfer) {
		vk_capture(data, queue, info, (VkImage)vk_image);
		return true;
	} else {
		if (!queue_data->supports_transfer &&
		    !queue_data->report_unsupports_transfer) {
		    hlog("vulkan capture is not supported");
		    queue_data->report_unsupports_transfer = true;
		}
		return false;
	}
}
