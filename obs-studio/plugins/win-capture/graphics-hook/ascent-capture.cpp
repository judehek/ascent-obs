...
static HMODULE ascent_handle = NULL;
...
{
	return (void *)GetProcAddress(ascent_handle, name);
}

static bool __cdecl ascent_d3d9_capture(void *device, void *surface)
{
	return capture_d3d9(device, surface);
}

static bool __cdecl ascent_d3d9_reset_hook()
{
	return reset_d3d9();
}

static bool __cdecl ascent_d3d11_capture(void *swap_chain,
					  void *backbuffer_ptr)
{
	return capture_d3d11(swap_chain, backbuffer_ptr);
}

static bool __cdecl ascent_d3d11_reset()
{
	return reset_d3d11();
}

static bool __cdecl ascent_d3d12_capture(void *swap_chain, void *queue_ptr)
{
	return capture_d3d12(swap_chain, queue_ptr);
}

static bool __cdecl ascent_d3d12_reset()
{
	return reset_d3d12();
}

static bool __cdecl ascent_ogl_capture(HDC hdc)
{
	return capture_ogl(hdc);
}

static bool __cdecl ascent_ogl_reset(HGLRC hrc)
{
...
{
	if (!ascent_handle) {
		ascent_handle = GetModuleHandleW(L"ascentclient");
	}
	...
	static_cast<callback_type2>(get_proc("set_d3d9_capture_callbacks"))(
		 &ascent_d3d9_capture, &ascent_d3d9_reset_hook);

	static_cast<callback_type2>(
		get_proc("set_d3d11_capture_callbacks"))(&ascent_d3d11_capture,
							 &ascent_d3d11_reset);
	static_cast<callback_type2>(
		get_proc("set_d3d12_capture_callbacks"))(&ascent_d3d12_capture,
							 &ascent_d3d12_reset);

	static_cast<callback_type2>(get_proc("set_ogl_capture_callbacks"))(
		&ascent_ogl_capture, &ascent_ogl_reset);
