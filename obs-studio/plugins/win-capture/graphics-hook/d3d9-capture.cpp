...
static inline bool d3d9_shtex_init_copytex()
{
	...
	if (offsets.d3d9_clsoff && offsets.is_d3d9ex_clsoff) {
		...
		if (d3d9_ptr) {
			p_is_d3d9 =
				(BOOL *)(d3d9_ptr + offsets.is_d3d9ex_clsoff);
		}
	} else {
...
static void d3d9_init(IDirect3DDevice9 *device)
{
	...
	if (global_hook_info->force_shmem ||
	    (!d3d9ex && data.patch == -1 && !has_d3d9ex_bool_offset)) {
     	hlog("d3d9_shtex_capture: trying shmem init");
		success = d3d9_shmem_init(window);
	} else {
    	static int shtex_first_attempt = GetTickCount();
		success = d3d9_shtex_init(window);
		if (!success &&
		    GetTickCount() - shtex_first_attempt >
			    SWITCH_TO_SHM_CAPTURE_FALLBACK_TIMEUOT) {
			hlog_hr("d3d9_shtex_capture: failed to init, switch to shmem",
				hr);
	...
	if (!success) {
    		hlog_hr("d3d9_init: failed to init", hr);
		d3d9_free();
	}
}
...
bool capture_d3d9(void *device, void *surface)
{
	...
	static bool d3d9_capture_printed = false;
	if (!d3d9_capture_printed && pipe.handle) {
		hlog("capture_d3d9");
		d3d9_capture_printed = true;
	}

	IDirect3DDevice9 *d3d_device = static_cast<IDirect3DDevice9 *>(device);

	IDirect3DSurface9 *d3d_surface =
		static_cast<IDirect3DSurface9 *>(surface);

	d3d9_capture(d3d_device, d3d_surface);
	return true;
}
