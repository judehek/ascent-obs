...
log_message_callback external_log_callback = NULL;
...
extern bool was_init = false;
extern bool on_demand_init_reties = 10;
...
static inline void hlogv(const char *format, va_list args)
{
	...
	if (num) {
		...
		if (external_log_callback != 0) {
			__try {
				(external_log_callback)(message, 0);
			} // Handle any exceptions by continuing from this point.
			__except (EXCEPTION_EXECUTE_HANDLER) {
				external_log_callback = 0;
			}
		}

		if (&pipe &&
		    !ipc_pipe_client_write(&pipe, message, (size_t)num + 1)) {
			if (&pipe) {
				ipc_pipe_client_free(&pipe);
				//init_pipe();
			}
...
static inline bool init_shared_info(
	size_t size, HWND window, HWND* hooked_window)
{
	wchar_t name[64];
	*hooked_window = GetAncestor(window, GA_ROOT);

	swprintf(name, 64, SHMEM_TEXTURE "_%" PRIu32 "_%u",
		 (uint32_t)(uintptr_t)(*hooked_window), ++shmem_id_counter);
			...
			uintptr_t handle)
{
	...
	*data = shmem_info;
	...
	global_hook_info->window = (uint32_t)(uintptr_t)hooked_window;
			...
			uint32_t cy, uint32_t pitch, uint32_t format, bool flip)
{
	...
	if (!init_shared_info(total_size, window, &hooked_window)) {
...
void set_external_log_callback(log_message_callback callback) {
	external_log_callback = callback;
}
