...
#define SETTING_GAME_IN_FOREGROUND    "in_foreground"
...
#define SETTING_MODE_PROCESS     "process"
...
char *get_common_program_file_module(const char *file)
{
	...
	snprintf(path, MAX_PATH * 2, "%s\\overwolf\\ow-obs",
...
struct game_capture {
	...
	bool cursor_hidden;
	bool pause;
	bool force_shmem_internal;
	bool was_ever_active;
	...
	uint32_t open_sht_error_counter;
...
bool HasProcess(DWORD process_id)
{
	...
	//for elevated process
	PROCESSENTRY32 entry;
	entry.dwSize = sizeof(PROCESSENTRY32);
	HANDLE snapshot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
	if (Process32First(snapshot, &entry) == TRUE) {
		while (Process32Next(snapshot, &entry) == TRUE && !result) {
...
{
	if (&gc->pipe) {
		//info("closing pipe %d", gc->process_id);
		ipc_pipe_server_free2(&gc->pipe, 2000);
		//info("pipe closed", gc->process_id);
	}
	...
	if (gc->process_id > 0) {
		HANDLE handle = open_process(SYNCHRONIZE, false, gc->process_id);
		process_alive = handle != INVALID_HANDLE_VALUE;
		if (handle != INVALID_HANDLE_VALUE) {
			CloseHandle(handle);
		}

		if (!process_alive) {
			warn("game process (%d) is down", gc->process_id);
		}
	}

	if (gc->capturing || error || !process_alive) {
		gc->capturing = false;
		obs_source_update_capture(gc->source, false, false, error);
	}
			      ...
			      obs_data_t *settings, const char *window)
{
	...
	else if (mode_str && strcmp(mode_str, SETTING_MODE_PROCESS) == 0)
		cfg->mode = CAPTURE_MODE_PROCESS;
	else
	...
	cfg->process_id =
		(uint32_t)obs_data_get_int(settings, SETTING_PROCESS_ID);
	cfg->hook_rate =
				       ...
				       struct game_capture_config *cfg2)
{
	if (cfg1->mode != cfg2->mode) {
		...
		return true;
	} else if (cfg1->process_id != cfg2->process_id) {
		return true;
	}
...
static void game_capture_update(void *data, obs_data_t *settings)
{
	...
	cfg.force_shmem |= gc->force_shmem_internal;

	reset_capture = capture_needs_reset(&cfg, &gc->config);

	bool pause = !obs_data_get_bool(settings, SETTING_GAME_IN_FOREGROUND);

	if (gc->pause != pause) {
		warn("game capture pause state changed: %d -> %d", gc->pause,
		     pause);
	}
	...
	bool force_shmem_internal = gc->force_shmem_internal;
	free_config(&gc->config);
	gc->config = cfg;
	gc->config.force_shmem |= force_shmem_internal;
	...
	if (cfg.mode == CAPTURE_MODE_PROCESS && gc->process_id == 0) {
		gc->process_id = cfg.process_id;
	}

	if (!gc->initial_config) {
		if (reset_capture) {
			info("stop_capture: game_capture_update");
			stop_capture(gc);
		}
	} else {
...
static void *game_capture_create(obs_data_t *settings, obs_source_t *source)
{
	...
	gc->open_sht_error_counter = 0;
	gc->pause = false;
	gc->force_shmem_internal = false;
	gc->need_to_retry = 0;
	gc->last_retry_ts = 0;
	gc->was_ever_active = false;
...
static inline bool init_hook_info(struct game_capture *gc)
{
	...
	gc->global_hook_info->ready_to_capture = true;
	debug("init_hook_info: ready to capture");
	obs_leave_graphics();
...
static void pipe_log(void *param, uint8_t *data, size_t size)
{
	...
	if (data && size)
		info("[Game Log]: %s", data);
}

static inline bool init_pipe(struct game_capture *gc)
{
	// TODO (bfox): understand why calling ipc_pipe_server_free(&gc->pipe);  (on capture_stop)
	// is hang
	...
	//if (&gc->pipe && gc->pipe.handle) {
	//  warn("init_pipe: already created");
	// return true;
	//}
...
{
	if (!os_file_exists_w(inject_path_w)) {
		warn("hook remote: file not found");
		return false;
	}
	SHELLEXECUTEINFO execInfo = {0};
	execInfo.cbSize = sizeof(execInfo);
	execInfo.lpFile = inject_path_w;
	execInfo.lpParameters = command_line_w;
					 ...
					 bool use_shell_execute)
{
	...
	wchar_t hook_path_abs_w[MAX_PATH];
	wchar_t *path_ret = _wfullpath(hook_path_abs_w, hook_dll_w, MAX_PATH);

	wchar_t inject_path_abs_w[MAX_PATH];
	_wfullpath(inject_path_abs_w, inject_path_w, MAX_PATH);
	...
	} else { // try shell execute
		swprintf(command_line_w, 4096, L"\"%s\" %lu %lu %lu",
			 hook_path_abs_w, (unsigned long)anti_cheat,
			 anti_cheat ? gc->thread_id : gc->process_id,
			 gc->process_id);
		success = create_inject_process_shell(
			gc,
			use_shell_execute ? inject_path_w : inject_path_abs_w,
			command_line_w);
		if (!success) {
			warn("Failed to create inject helper process: %lu",
			     GetLastError());
		} else {
			auto pid = ms_get_process_id(gc->process_is_64bit ?
				"inject-helper64.exe" : "inject-helper32.exe");
			gc->injector_process = pid
				? OpenProcess(SYNCHRONIZE, FALSE, pid) : 0;
...
static inline bool inject_hook(struct game_capture *gc)
{
	...
	if (gc->injection_retries++ >= kMaxInjectionHooksRetries) {
		...
		if ((kMaxInjectionHooksRetries+1) == gc->injection_retries) {
			warn("to many injection hooks retrites, terminating capture");
			stop_capture_with_error(gc, "inject_helper_timeout_error");
		}
		return false;
	}
	// end overwolf
...
static bool init_hook(struct game_capture *gc)
{
	...
	} else if (gc->config.mode != CAPTURE_MODE_PROCESS) {
		if (ms_get_window_exe(&exe, gc->next_window)) {
			info("attempting to hook process: %s", exe.array);
		}
	} else if (gc->config.mode == CAPTURE_MODE_PROCESS) {
		if (!HasProcess(gc->process_id))
			return false;
	}
	...
	if (gc->config.mode != CAPTURE_MODE_PROCESS && false) {
		if (!open_target_process(gc)) {
			return false;
		}
	} else {
		if (!gc->target_process) {
			open_target_process(gc);
		}
	}
	...
	if (!attempt_existing_hook(gc)) {
		...
		warn("attempt_existing_hook: failure");
		return false;
	}
...
static void try_hook(struct game_capture *gc)
{
	...
	if (gc->config.mode == CAPTURE_MODE_PROCESS) {
		if (gc->process_id != 0) {
			...
			warn("game capture process is NULL");
			gc->active = false;
		}
		return;
	}
...
static inline enum capture_result init_capture_data(struct game_capture *gc)
{
	...
	if (!init_data_map(gc, (HWND)(uintptr_t)gc->global_hook_info->window)) {
		HWND retry_hwnd = (HWND)(uintptr_t)gc->window;
		error = GetLastError();

		/* if there's an error, just override.  some windows don't play
		 * nice. */
		if (retry_hwnd && init_data_map(gc, retry_hwnd)) {
			error = 0;
		}
	}
...
static bool start_capture(struct game_capture *gc)
{
	...
	if (gc->global_hook_info->type == CAPTURE_TYPE_MEMORY) {
		...
		if (!init_shtex_capture(gc)) {
			if (++gc->open_sht_error_counter > 1) {
				warn("Switching to force shmem capture");
				stop_capture(gc);
				gc->config.force_shmem = true;
				gc->force_shmem_internal = true;
...
static void game_capture_tick(void *data, float seconds)
{
	...
	bool deactivate = os_atomic_set_bool(&gc->deactivate_hook, false) &&
			  gc->config.mode != CAPTURE_MODE_PROCESS;
	...
	if (!obs_source_showing(gc->source)) {
		if (gc->showing) {
			if (gc->active && !gc->pause) {
				info("stop capture not active: %d",
				     gc->process_id);
				stop_capture(gc);
			}
			gc->showing = false;
		...
		} else if (gc->pause && gc->active &&
			   !capture_valid(gc)) {
			info("paused capture window no longer exists terminating capture");
			stop_capture(gc);
		}
		return;
	...
	if (gc->injector_process &&
	        object_signalled_with_timeout(gc->injector_process, 2000)) {
		DWORD exit_code = 0;
		...
		if (exit_code != 0) {
			...
			gc->retry_interval =
				...
				hook_rate_to_float(gc->config.hook_rate);
			info("stop capture !gc->capturing: %d", gc->process_id);
			stop_capture(gc);
		}
	}

	bool hook_ready = gc->hook_ready && object_signalled(gc->hook_ready);
	bool hook_retry = gc->hook_ready && gc->need_to_retry > 0;
	if (hook_ready || hook_retry) {
		debug("capture initializing!");
		enum capture_result result = init_capture_data(gc);
		if (hook_ready && gc->need_to_retry == 0) {
			info("capture initializing result: %d", result);
		}
		if (result == CAPTURE_SUCCESS) {
			gc->capturing = start_capture(gc);
			if (gc->capturing) {
				obs_source_update_capture(
					gc->source, true,
					gc->global_hook_info->force_shmem ||
						gc->global_hook_info->type ==
							CAPTURE_TYPE_MEMORY,
					NULL);
				gc->need_to_retry = 0;
			} else {
			...
			// wait at least 5000 * 10 before failing the capture
			const int kRetryIntervalCount = 5000;
			if (hook_ready && gc->need_to_retry == 0) {
				warn("capture initializing failure");
			}
			debug("init_capture_data failed");
			if (result == CAPTURE_RETRY &&
			    (GetTickCount64() - gc->last_retry_ts) >
				    kRetryIntervalCount) {
				gc->need_to_retry++;
				gc->last_retry_ts = GetTickCount64();
				warn("init_capture_data failed %d",
				     gc->need_to_retry);
			}
		}

		const int kCaptureDataErrorRetries = 20;
		if (gc->capturing) {
			if (gc->config.mode == CAPTURE_MODE_ANY) {
		...
		if ((result != CAPTURE_RETRY && !gc->capturing) ||
			...
			gc->retry_interval =
				...
				hook_rate_to_float(gc->config.hook_rate);
			stop_capture(gc);
			if (gc->need_to_retry > kCaptureDataErrorRetries) {
				info("stop capture ERROR_RETRY_INTERVAL: %d",
					gc->process_id);
				stop_capture_with_error(gc, "capture_data_error");
			}
		}
	}
...
{
	struct game_capture *gc = data;
	return (gc->active && gc->capturing && gc->global_hook_info) ? gc->global_hook_info->cx : 0;
}

static uint32_t game_capture_height(void *data)
{
	struct game_capture *gc = data;
	return (gc->active && gc->capturing && gc->global_hook_info) ? gc->global_hook_info->cy : 0;
}
