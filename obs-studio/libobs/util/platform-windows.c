...
int os_get_local_config_path(char *dst, size_t size, const char *name)
{
	return os_get_path_internal(dst, size, name, CSIDL_LOCAL_APPDATA);
}

char *os_get_local_config_path_ptr(const char *name)
{
	return os_get_path_ptr_internal(name, CSIDL_LOCAL_APPDATA);
}
...
bool get_reg_dword(HKEY hkey, LPCWSTR sub_key, LPCWSTR value_name,
		   struct reg_dword *info)
{
	...
	*info = reg;
	return reg.status == ERROR_SUCCESS;
}

bool get_reg_string(HKEY hkey, LPCSTR sub_key, LPCSTR value_name,
		   struct reg_sz *info)
{
	struct reg_sz reg = {0};
	HKEY key;
	...
	status = RegOpenKeyExA(hkey, sub_key, 0, KEY_READ, &key);
	...
	reg.size = sizeof(reg.return_value);
	reg.status = RegQueryValueExA(key, value_name, NULL, NULL,
				      (LPBYTE)&reg.return_value, &reg.size);
	RegCloseKey(key);

	if (reg.status != ERROR_SUCCESS) {
		info->status = reg.status;
		info->size = 0;
		return false;
	}
	...
	info->size = reg.size;
	info->status = reg.status;
	reg.return_value[reg.size - 1] = 0;
	strcpy(info->return_value, reg.return_value);
...
uint64_t os_get_epoch_time(void) {
  FILETIME              dummy;
  GetSystemTimeAsFileTime(&dummy);
  return (((uint64_t)dummy.dwHighDateTime) << 32) + dummy.dwLowDateTime;
}
...
uint64_t os_get_epoch_time_unix(void) {
  uint64_t systemtime = os_get_epoch_time();
  systemtime -= NANO_SECS_INTERVAL_UNITS_BETWEEN_1601_AND_1970; // we start at 1601 and above
  systemtime /= 10; // 100-nanosecs to microsecs
  systemtime /= kMicrosecondsPerMillisecond;
  return systemtime;
}
