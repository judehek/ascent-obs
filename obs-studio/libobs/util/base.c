...
OBS_NORETURN static void def_crash_handler(void * exception, const char *format, va_list args,
				       void *param)
{
...
static void(*crash_handler)(void *, const char *, va_list, void *) = def_crash_handler;
...
void base_set_crash_handler(void (*handler)(void*, const char *, va_list, void *),
			    void *param)
{
	crash_param = param;
...
OBS_NORETURN void bcrash(const char *format, ...)
{
	...
	crash_handler(0, format, args, crash_param);
	va_end(args);
}

OBS_NORETURN void bcrash1(void *exception, const char *format, ...)
{
	va_list args;
	...
	crashing = 1;
	va_start(args, format);
	crash_handler(exception, format, args, crash_param);
	va_end(args);
	exit(0);
}
