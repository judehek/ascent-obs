...
T TrimStringPieceT(T input, T trim_chars, TrimPositions positions)
{
	...
	size_t end = (positions & TRIM_TRAILING)
			     ...
			     : input.size();	
	return input.substr(std::min<size_t>(begin, input.size()), end - begin);
}

std::wstring_view TrimWhitespace(std::wstring_view input,
				 TrimPositions positions)
{
	return TrimStringPieceT(input, std::wstring_view(kWhitespaceWide),
				positions);
}
...
void AppendSwitchesAndArguments(CommandLine* command_line,
  ...
  for (size_t i = 1; i < argv.size(); ++i) {
    auto arg = CommandLine::StringType(TrimWhitespace(argv[i], TRIM_ALL));
...
void CommandLine::SetProgram(const std::wstring& program) {
  StringType program_copy = StringType(TrimWhitespace(program, TRIM_ALL));
...
void CommandLine::ParseFromString(const CommandLine::StringType& command_line) {
  auto command_line_string =
		CommandLine::StringType(TrimWhitespace(command_line, TRIM_ALL));
