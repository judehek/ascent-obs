...
static const char *output_signals[] = {
	...
	"void replay_ready(ptr output)",
	"void replay_error(ptr output)",
	"void replay_warning(ptr output)",
	"void disk_space_warning(ptr output)",
	"void video_split(ptr output)",
	"void replay_fully_armed(ptr output)",
	NULL,
};
...
bool is_encoder_valid(obs_encoder_t* encoder)
{
	bool is_valid = obs_encoder_initialize(encoder);

	const char* last_error = obs_encoder_get_last_error(encoder);
	char* error = bstrdup(last_error);
	
	const char *last_code = obs_encoder_get_last_code(encoder);
	char *code = bstrdup(last_code);

	obs_encoder_shutdown(encoder);

	obs_encoder_set_last_error(encoder, error);
	obs_encoder_set_last_code(encoder, code);
...
uint64_t obs_output_get_video_encoder_pts(const obs_output_t *output)
{
...
int obs_output_get_info_drawn_frame(const obs_output_t *output) {
  if (!obs_output_valid(output, "obs_output_get_info_drawn_frame"))
    return 0;

  struct obs_core_video *video = &obs->video;
  return video->total_frames - output->starting_drawn_count;
}

int obs_output_get_info_lagged_frame(const obs_output_t *output) {
  if (!obs_output_valid(output, "obs_output_get_info_lagged_frame"))
    return 0;

  struct obs_core_video *video = &obs->video;
  return video->lagged_frames - output->starting_lagged_count;
}
...
static inline void signal_stop(struct obs_output *output)
{
	...
	calldata_set_ptr(&params, "output", output);
	calldata_set_int(&params, "duration", output->duration);
...
static void obs_output_end_data_capture_internal_new(obs_output_t *output,
						 bool signal, int64_t duration)
{
	int ret;
...
static void obs_output_end_data_capture_internal(obs_output_t *output,
						 bool signal)
{
	obs_output_end_data_capture_internal_new(output, signal, 0);
}
...
void obs_output_end_data_capture1(obs_output_t *output, int64_t duration)
{
	obs_output_end_data_capture_internal_new(output, true, duration);
}
...
void obs_output_signal_replay_ready(struct obs_output* output,
  int64_t video_duration,
  int64_t video_system_start_time, 
  const char* path) {
  if (!obs_output_valid(output, "obs_output_signal_replay_ready"))
    return;

  struct calldata params = { 0 };
  calldata_set_int(&params, "duration", video_duration);
  calldata_set_int(&params, "system_start_time", video_system_start_time);
  calldata_set_string(&params, "path", path);
  calldata_set_ptr(&params, "output", output);
  signal_handler_signal(output->context.signals, "replay_ready", &params);
  calldata_free(&params);
}

void obs_output_signal_video_split(struct obs_output* output,
  int64_t video_duration_from_start,
  int64_t video_duration, 
  int64_t video_last_frame_epoch_timestamp,
  const char* path, 
  const char* next_file_path) {
  if (!obs_output_valid(output, "obs_output_signal_video_split"))
    return;

  struct calldata params = { 0 };
  calldata_set_int(&params, "duration", video_duration_from_start);
  calldata_set_int(&params, "split_video_duration", video_duration);
  calldata_set_int(&params, "last_frame_ts", video_last_frame_epoch_timestamp);
  calldata_set_string(&params, "path", path);
  calldata_set_string(&params, "next_file_path", 
    next_file_path ? next_file_path : "");

  calldata_set_ptr(&params, "output", output);
  signal_handler_signal(output->context.signals, "video_split", &params);
  calldata_free(&params);
}

void obs_output_signal_replay_error(struct obs_output* output,
  const char *path, const char *error) {
  if (!obs_output_valid(output, "obs_output_signal_replay_error"))
    return;

  struct calldata params = { 0 };
  calldata_set_string(&params, "path", path);
  calldata_set_string(&params, "error", error);
  calldata_set_ptr(&params, "output", output);
  signal_handler_signal(output->context.signals, "replay_error", &params);
  calldata_free(&params);
}

void obs_output_signal_replay_warning(struct obs_output *output,
				      const char *path,
				      const char *warning)
{
	if (!obs_output_valid(output, "obs_output_signal_replay_warning"))
		return;

	struct calldata params = {0};
	calldata_set_string(&params, "path", path);
	calldata_set_string(&params, "warning", warning);
	calldata_set_ptr(&params, "output", output);
	signal_handler_signal(output->context.signals,"replay_warning",&params);
	calldata_free(&params);
}

void obs_output_signal_disk_space_warning(struct obs_output *output,
					  const char *path,const char *warning)
{
	if (!obs_output_valid(output,"obs_output_signal_disk_space_warning")) {
		return;
	}		
...
void obs_output_signal_replay_fully_armed(struct obs_output *output)
{
	if (!obs_output_valid(output, "obs_output_signal_replay_fully_armed"))
		return;
	...
	calldata_init_fixed(&params, stack, sizeof(stack));
	calldata_set_ptr(&params, "output", output);
	signal_handler_signal(output->context.signals, "replay_fully_armed",
			      &params);
}
