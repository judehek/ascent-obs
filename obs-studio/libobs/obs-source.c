...
static const char *source_signals[] = {
	...
	"void update_capture_state(ptr source, bool capture)",
...
void obs_source_update_capture(obs_source_t *source, bool capture,
	bool sli_compatibility, const char* error) {
  if(!obs_source_valid(source, "obs_source_update_capture_state"))
    return;

  struct calldata data;
  calldata_init(&data);
  calldata_set_ptr(&data, "source", source);
  calldata_set_bool(&data, "capture", capture);
  calldata_set_bool(&data, "sli_compatibility", sli_compatibility);
  if (error) {
	calldata_set_string(&data, "error", error);
  }

  signal_handler_signal(source->context.signals, "update_capture_state", &data);
  calldata_free(&data);
