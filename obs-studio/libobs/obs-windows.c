...
bool initialize_com(void)
{
	...
	{
		blog(LOG_INFO, "CoInitializeEx success");
	}
...
void get_system_info(obs_data_t* out_data) {
  if (win_ver < 0xA00 || !out_data)
    return;
  ...
  get_win_ver(&ver);
  if (game_mode_enabled.status != ERROR_SUCCESS &&
      ver.major >= 10 && ver.build >= 17763) {  // build 1809
    // in case regisry is not for assuming is on
    game_mode_enabled.return_value = true;
  }

  if (game_bar_enabled.status == ERROR_SUCCESS) {
    obs_data_set_bool(out_data,
                      "game_bar_enabled",
                      (bool)game_bar_enabled.return_value);
  }

  if (game_dvr_allowed.status == ERROR_SUCCESS) {
    obs_data_set_bool(out_data,
                      "game_dvr_allowed",
                      (bool)game_dvr_allowed.return_value);
  }

  if (game_dvr_enabled.status == ERROR_SUCCESS) {
    obs_data_set_bool(out_data,
                      "game_dvr_enabled",
                      (bool)game_dvr_enabled.return_value);
  }

  if (game_dvr_bg_recording.status == ERROR_SUCCESS) {
    obs_data_set_bool(out_data,
                      "game_dvr_bg_recording",
                      (bool)game_dvr_bg_recording.return_value);
  }

  obs_data_set_bool(out_data,
                    "game_mode_enabled",
                    (bool)game_mode_enabled.return_value);

    obs_data_set_bool(out_data, "hw_sch_mode", (bool)hags_enabled.return_value);
