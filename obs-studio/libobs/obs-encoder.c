...
void obs_encoder_shutdown(obs_encoder_t *encoder)
{
	...
	obs_encoder_set_last_code(encoder, NULL);
	pthread_mutex_unlock(&encoder->init_mutex);
}
...
bool do_encode(struct obs_encoder *encoder, struct encoder_frame *frame)
{
	...
	pkt.encoder = encoder;
	pkt.sys_pts_usec = (frame->sys_pts /*/ 1000LL*/);
...
static void receive_video(void *param, struct video_data *frame)
{
	...
	enc_frame.sys_pts = frame->timestamp;
	encoder->last_ts = frame->timestamp;
...
const char *obs_encoder_get_last_code(obs_encoder_t *encoder)
{
	if (!obs_encoder_valid(encoder, "obs_encoder_get_last_code"))
		return NULL;

	return encoder->last_error_code;
}

void obs_encoder_set_last_code(obs_encoder_t *encoder, const char *code)
{
	if (!obs_encoder_valid(encoder, "obs_encoder_set_last_code"))
		return;

	if (encoder->last_error_code)
		bfree(encoder->last_error_code);

	if (code)
		encoder->last_error_code = bstrdup(code);
	else
		encoder->last_error_code = NULL;
}
