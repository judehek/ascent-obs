...
#if defined(_WIN32) 
extern void get_system_info(obs_data_t* out_data);
#endif
...
//void set_commandline_parser(struct obs_command_line_parser *parser)
//{
//	command_line_parser = parser;
//}
//
//struct obs_command_line_parser *get_commandline_parser()
//{
...
void get_system_game_info(obs_data_t* out_data) {
#if defined(_WIN32) 
  get_system_info(out_data);
#endif 
}
...
void obs_set_video_levels(float sdr_white_level, float hdr_nominal_peak_level)
{
	...
	blog(LOG_INFO, "SDR: %d HDR: %d", sdr_white_level,
...
EXPORT bool obs_get_enum_video_adapters(
  ...
  return enum_adapters(video->graphics, callback, param);
}
...
static gs_texture_t* obs_render_main_texture_internal(enum gs_blend_type src_c,
...
gs_texture_t * obs_render_main_texture(void)
{
	return 	obs_render_main_texture_internal(GS_BLEND_ONE, GS_BLEND_INVSRCALPHA,
...
bool obs_wait_for_destroy_queue(void)
{
	...
	if (!obs || !obs->video.thread_initialized || !obs->audio.audio)
		return false;
...
EXPORT void *get_graphics_information(bool refresh, int *error) {
	...
	struct obs_core_video *video = &obs->video;
	gs_enter_context(video->graphics);
	void * res = gs_get_graphics_information(refresh);
	gs_leave_context();
	return res;
}
