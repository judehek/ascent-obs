...
enum obs_flip_type {
	OBS_FLIP_NONE,			/**< no flip         */
	OBS_FLIP_VERTICAL,		/**< flip Vertical   */
	OBS_FLIP_HORIZONTAL,		/**< flip Horizontal */
};
...
/** @return json of all system game feature data*/
EXPORT void get_system_game_info(obs_data_t *out_data);
...
/** Gets machine video adapters */
EXPORT bool obs_get_enum_video_adapters(bool (*callback)(void *param,
										const char *name,
							 			uint32_t id),
										void *param);
...
EXPORT gs_texture_t *obs_render_main_texture(void);
...
EXPORT bool is_encoder_valid(obs_encoder_t *encoder);
...
/** Returns the current video encoder last presentation ts associated with this output */
EXPORT uint64_t obs_output_get_video_encoder_pts(const obs_output_t *output);
...
EXPORT int obs_output_get_info_drawn_frame(const obs_output_t *output);
EXPORT int obs_output_get_info_lagged_frame(const obs_output_t *output);
/**
...
/** Ends data capture from media/encoders */
EXPORT void obs_output_end_data_capture1(obs_output_t *output,
					 int64_t duration);
...
/**
* Signals that the replay output video is ready.
*
* @param  output  Output context
* @param  code    Error code (or OBS_OUTPUT_SUCCESS if not an error)
*/
EXPORT void obs_output_signal_replay_ready(obs_output_t *output,
					   int64_t video_duration,
					   int64_t video_system_start_time,
					   const char *path);

/**
* Signals that the video split output video is ready.
*
* @param  output  Output context
* @param  code    Error code (or OBS_OUTPUT_SUCCESS if not an error)
*/
EXPORT void obs_output_signal_video_split(
	obs_output_t *output, int64_t video_duration_from_start,
	int64_t video_duration, int64_t video_last_frame_epoch_timestamp,
	const char *path, const char *next_file_path);

/**
* Signals that the replay output video has error.
*
* @param  output  Output context
* @param  code    Error code (or OBS_OUTPUT_SUCCESS if not an error)
*/
EXPORT void obs_output_signal_replay_error(obs_output_t *output,
					   const char *path, const char *error);

/**
* Signals that the replay output video has warning.
*
* @param  output  Output context
* @param  code    Error code (or OBS_OUTPUT_SUCCESS if not an error)
*/
EXPORT void obs_output_signal_replay_warning(obs_output_t *output,
					     const char *path,
					     const char *warning);

/**
* Signals that the output has disk space warning.
*
* @param  output  Output context
* @param  code    Error code (or OBS_OUTPUT_SUCCESS if not an error)
*/
EXPORT void obs_output_signal_disk_space_warning(obs_output_t *output,
						 const char *path,
						 const char *warning);

/**
* Signals that the replay buffer fully armed .
*
* @param  output  Output context
* @param  code    Error code (or OBS_OUTPUT_SUCCESS if not an error)
*/
EXPORT void obs_output_signal_replay_fully_armed(obs_output_t *output);
...
EXPORT const char *obs_encoder_get_last_code(obs_encoder_t *encoder);
EXPORT void obs_encoder_set_last_code(obs_encoder_t *encoder, const char *code);
