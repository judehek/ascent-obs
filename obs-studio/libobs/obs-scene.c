...
static void update_item_transform(struct obs_scene_item *item, bool update_tex)
{
	...
	if (item->flip_type != OBS_FLIP_NONE) {
		matrix4_scale3f(
			&item->draw_transform, &item->draw_transform,
			item->flip_type == OBS_FLIP_HORIZONTAL ? -1.0f : 1.0f,
			item->flip_type == OBS_FLIP_VERTICAL ? -1.0f : 1.0f,
			1.0f);
	}
	...
	if (item->flip_type == OBS_FLIP_NONE) {
		matrix4_translate3f(&item->draw_transform,
				    &item->draw_transform, item->pos.x,
				    item->pos.y, 0.0f);
	} else {
		matrix4_translate3f(
			&item->draw_transform, &item->draw_transform,
			item->flip_type == OBS_FLIP_HORIZONTAL ? item->bounds.x
							       : item->pos.x,
			item->flip_type == OBS_FLIP_VERTICAL ? item->bounds.y
							     : item->pos.y,
			0.0f);
	}
