...
EXPORT int os_get_local_config_path(char *dst, size_t size, const char *name);
EXPORT char *os_get_local_config_path_ptr(const char *name);
...
#ifdef _WIN32
EXPORT uint64_t os_get_epoch_time(void);
EXPORT uint64_t os_get_epoch_time_unix(void);
#endif
