...
static inline bool ipc_pipe_internal_create_events(ipc_pipe_server_t *pipe)
{
  ...
  pipe->exit_event = CreateEvent(NULL, false, false, NULL);
  ...
  size_t new_size)
{
  ...
  pipe->read_data = (uint8_t*)realloc(pipe->read_data, new_size);
...
{
  ipc_pipe_server_t *pipe = (ipc_pipe_server_t*)param;
  ...
  size_t size)
{
  ...
  bool res = !!WriteFile(pipe->handle, data, (DWORD)size, &bytes, NULL);
  if (!res) {
    return -4;
  }
